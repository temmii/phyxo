...
class Plugin extends Repository implements MetadataInitializer
{
    ...
    public function changeState($id, $state='active') {
        $query = $this->getQuery('UPDATE '. $this->metadata->getTable() .' SET state = :state WHERE id = :id');
        $query->setParams(['id' => $id, 'state' => $state]);
        $query->execute();
    }
